...
public class New extends Expression {
    ...
    public ScopeContext ntcGenCons(NTCEnv env, ScopeContext parent) {
        ...
        Sym sym = env.getCurSym(contractName);
        assert sym instanceof ContractSym : "Not a contract name: " + location.errString();
        FuncSym constructorSym = ((ContractSym) sym).getConstructorSym();
        assert !env.inConstructor() || env.superCalled() : "should not call methods before called super in constructor: " + contractName + " at " + location.toString();
        assert constructor_call.args.size() == constructorSym.parameters.size() : "number of values provided does not match the number of arguments of the called method: " + contractName + " at " + location.toString();
